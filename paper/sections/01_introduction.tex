...
While this overhead is negligible for small datasets, comparison-driven partitioning can introduce substantial pipeline inefficiency at scales involving millions of elements \cite{kaligosiHowBranchMispredictions2006}. Recent efforts like BlockQuicksort \cite{edelkampBlockQuicksortHowBranch2016} mitigate this by decoupling control from data flow, yet they still fundamentally rely on $O(N \log N)$ comparison operations.
...
\begin{axis}[
 ...
 symbolic x coords={Random, Gaussian, Nearly Sorted, Zipfian, Duplicates, Strings},
 xtick=data,
...
\addplot coordinates {(Random, 2.39) (Gaussian, 1.78) (Nearly Sorted, 2.45) (Zipfian, 0.58) (Duplicates, 0.93) (Strings, 4.77)};
\end{axis}
\end{tikzpicture}
...
First, I introduce Aero (ARS Aero), a quantile-adaptive architecture that utilizes a cache-resident 1024-entry division-free lookup table to improve robustness under skewed and non-uniform distributions.
...
In addition, I introduce ARS Exp D, a streaming architecture based on epoch micro-batching and concurrent spatial consolidation.
...
Finally, I provide an extensive hardware-level evaluation of the ARS framework across multiple workload classes and data distributions.
My analysis examines throughput scaling, cache behavior, branch efficiency, latency stability, streaming interference, and memory-bandwidth limitations on modern multi-core hardware.
